...
const ContactList = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await fetch(`${window.location.origin}/server/contact/displaycontact`);
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${window.location.origin}/server/contact/deletecontact/${id}`, {
        method: 'DELETE',
      });
